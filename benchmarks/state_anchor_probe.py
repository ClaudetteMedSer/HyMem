...
def run_probe(
    ...
    *,
    ...
    for q in queries:
        ...
        # Expansion: the digest anchor's selection (profile rows FIRST, edges
        # fill the remainder) -> seed terms -> search, per source with its own
        # counter (Plan D correction 1). FTS legs per source are free; the
        # optional VECTOR call runs ONCE over the combined seed text (C3:
        # <=1 vector call per query).
        t0 = time.perf_counter()
        profiles, edges = select_state_anchor(conn, cap=cap)
...
def main() -> None:
    ...
    ap.add_argument("--cap", type=int, default=20,
                    help="anchor-edge cap (matches aggregation_digest_anchor_facts)")
    ...
    summary = run_probe(
        ...
        cap=args.cap, top_k=args.top_k, embedding_client=embedding_client,
    )
