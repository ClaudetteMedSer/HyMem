...
def add_peers(workspace_id: str, session_id: str, body: dict[str, Any]) -> list[dict]:
    ...
    Accepts two formats:
      - list:  {"peers": [{"id": "...", "metadata": {...}}, ...]}
      - dict:  {peer_id: {observe_me: bool, observe_others: bool}, ...}
    """
    hy = _get_hy()
    responses: list[dict] = []
    ...
    if "peers" in body:
        ...
        for entry in body["peers"]:
            ...
            hy.conn.execute(
                ...
                (peer_id, workspace_id, role, json.dumps(metadata)),
            )
            responses.append({
                "id": peer_id,
                "workspace_id": workspace_id,
        ...
        # Dict format (Honcho SDK normalize_peers_to_dict)
...
def peer_chat(workspace_id: str, peer_id: str, body: dict[str, Any]) -> dict:
    queries: list[str] = body.get("queries") or [body.get("query", "")]
    responses: list[str] = []
