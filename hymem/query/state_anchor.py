...
def select_state_anchor(conn: sqlite3.Connection, cap: int = 20) -> tuple[list, list]:
    """The digest anchor's selection, both legs: profile rows consume the cap
    FIRST, edges fill the remainder (``aggregate.py:_anchor_facts`` 801-835).
    """
    profiles = select_anchor_profile_rows(conn, cap=cap)
    edge_cap = max(0, cap - len(profiles))
    edges = select_anchor_edges(conn, cap=edge_cap)
