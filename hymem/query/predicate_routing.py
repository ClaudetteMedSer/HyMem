...
_KEYWORD_PREDICATES: dict[str, frozenset[str]] = {
    ...
    # personal-life value facts (v9)
    "owns": frozenset({"owns"}),
    "own": frozenset({"owns"}),
    "owned": frozenset({"owns"}),
    "possesses": frozenset({"owns"}),
    "drives": frozenset({"owns"}),
    "drive": frozenset({"owns"}),
    "lives in": frozenset({"located_in"}),
    "live in": frozenset({"located_in"}),
    "lives": frozenset({"located_in"}),
    "located in": frozenset({"located_in"}),
    "based in": frozenset({"located_in"}),
    "moved to": frozenset({"located_in"}),
    "located": frozenset({"located_in"}),
    "plays": frozenset({"participates_in"}),
    "play": frozenset({"participates_in"}),
    "practices": frozenset({"participates_in"}),
    "attends": frozenset({"participates_in"}),
    "enrolled in": frozenset({"participates_in"}),
    "participates in": frozenset({"participates_in"}),
    "age": frozenset({"has_attribute"}),
    "weight": frozenset({"has_attribute"}),
    "height": frozenset({"has_attribute"}),
    "salary": frozenset({"has_attribute"}),
}
